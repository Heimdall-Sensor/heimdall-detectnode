...
namespace Heimdall
{
    void DeviationDetection::calcDifference(Mat & image, Mat & diff_image) {
        //TODO: Use C++ templates instead of checks on depth?
        //TODO: Combine function with calcDeviation?
        ...
        for (std::vector<Mat>::size_type cond_idx = 0; cond_idx < d_model_means.size(); cond_idx++) {
            for (int i = 0; i < image.rows; i++) {
                for (int j = 0; j < image.cols; j++) {
                    ...
                    switch (d_depth) {
                        case CV_8U: 
                            result = abs(image.at<uchar>(i, j) - d_model_means[cond_idx].at<double>(i, j));
                            break;
                        case CV_16U: 
                            ...
                            if (image.at<short>(i, j) > 0 && d_model_counts[cond_idx].at<uint>(i, j) > 0) {
                                result = abs(image.at<short>(i, j) - d_model_means[cond_idx].at<double>(i, j));
                            } else {
                    ...
                    if (result > -1 && (cond_idx == 0 || (cond_idx > 0 && diff_image.at<double>(i, j) > result))) {
                        diff_image.at<double>(i, j) = result;
                    }
                }
            }
        }
